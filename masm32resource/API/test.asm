...
_WinMain		proc
		...
		assume fs:nothing 
			xor edx,edx
			mov ebx, fs:[edx + 30h] 	; ecx = address of PEB 
mov dl,0fch
sub dl,0f0h
mov ebx,[ebx+edx]


			mov ebx, [ebx + 1ch] 		; ecx = first entry in initialisation order list 
			mov ebx, [ebx] 				; ecx = second entry in list (kernel32.dll) 
			mov ebx, [ebx]
			mov ebp, [ebx + 08h] 		; ebp = base address of kernel32.dll 
