...
//再建一个类，专门来求inv
//At this time,I failed!
/*class HowToGetInv{
    private:
        long WhoWantGetInv,TheInvMod;
    public:
        long GetInv();
        void WayGetInv();
        void GcdisZero();
...
long HowToGetInv::GetInv(){
    long a[3]={1,0,TheInvMod},b[3]={0,1,WhoWantGetInv},t[3]={0,0,0},
            multiplier,CircleVary1,flag=0,InvResult=0;;                 //<-failed here; 
    while(flag==0){                                                     //so many varies    
        WayGetInv();                                                    //I cannot deliver them 
    }return InvResult;                                                  //to other public functions 
};                                                                      //in class HowToGetInv
void HowToGetInv::WayGetInv(){
    if(!b[2]){this.GcdisZero();break;}
    if(b[2]==1){this.GcdisOne();break;}
    this.GetMultiplier();
    this.UpdateCircle();
};
void HowToGetInv::GcdisZero(){
    InvResult=0;
};
void HowToGetInv::GcdisOne(){
    while(b[1]<0)
        b[1]+=InvMod;
    InvResult=b[1];
};
void HowToGetInv::GetMultiplier(){
    multiplier=a[2]/b[2];
};
void HowToGetInv::UpdateCircle(){
    for(CircleVary1=0;CircleVary1<3;CircleVary1++){
		t[CircleVary1]=a[CircleVary1]-b[CircleVary1]*multiplier;
		a[CircleVary1]=b[CircleVary1];
		b[CircleVary1]=t[CircleVary1];
	}
}
*/
int main(){
    ecc_point FirstPoint(0,6);//(9,9);
    int i=0;
    FirstPoint.show();
    for(i=1;i<20;i++)
    (i*FirstPoint).show();
    //(FirstPoint+FirstPoint).show();
    system("pause");
    return 0;   
}
